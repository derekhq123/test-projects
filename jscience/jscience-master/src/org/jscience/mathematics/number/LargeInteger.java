...
public final class LargeInteger extends Number<LargeInteger> {
    ...
    private static final ArrayFactory<LargeInteger> ARRAY_FACTORY = new ArrayFactory<LargeInteger>() {
        ...
        @Override
        public LargeInteger create(int capacity) {
     ...
     */
    public LargeInteger(int wordLength) {
    ...
    public LargeInteger high(int w) { // this.shiftRight(w * 63)
    ...
    public LargeInteger low(int w) { // this.minus(high(w).shiftLeft(w * 63));
        LargeInteger li = NO_ARRAY_FACTORY.object();
        li._words = _words;
        li._isNegative = _isNegative;
        for (int i = w; i > 0; i--) {
    ...
    public LargeInteger shiftWordLeft(int w) { // this.minus(high(w).shiftLeft(w * 63));
        if (_size == 0)
            return LargeInteger.ZERO;
    ...
    public void shiftRightSelf() {
        if (_size == 0)
            return;
        _size = Calculus.shiftRight(0, 1, _words, _size, _words);
    }

    public void shiftRightSelf(int n) {
        if ((n == 0) || (_size == 0))
            return;
    ...
    public void subtract(LargeInteger that) { // this >= that
    ...
    public boolean isShiftRightCorrection(int n) {
        int wordShift = n < 63 ? 0 : n / 63;
        int bitShift = n - ((wordShift << 6) - wordShift); // n - wordShift * 63
        int i = wordShift;
    ...
    public static long pow(int radix, int n) {
        if (radix == 10)
    ...
    public static Appendable write(LargeInteger li, int radix, int divisor,
	...
	public void set_words(long[] _words) {
		this._words = _words;
	}
	...
	public void set_isNegative(boolean _isNegative) {
		this._isNegative = _isNegative;
	}

	public void set_remainder(LargeInteger _remainder) {
