...
public final class SparseMatrix<F extends Field<F>> extends Matrix<F> {
    ...
    public FastTable<SparseVector<F>> getRows() {
        if (!_transposed)
            return _rows;
    ...
    private static final ObjectFactory<SparseMatrix> FACTORY = new ObjectFactory<SparseMatrix>() {
        ...
        @Override
        public void cleanup(SparseMatrix matrix) {
            matrix._rows.reset();
        }
    };
	...
	public void set_transposed(boolean _transposed) {
		this._transposed = _transposed;
	}

	public void set_zero(F _zero) {
		this._zero = _zero;
	}
