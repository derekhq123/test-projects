...
public final class ProductUnit<Q extends Quantity> extends DerivedUnit<Q> {
     ...
     */
    public ProductUnit(Element[] elements) {
        _elements = elements;
    }
    ...
    public static Unit<? extends Quantity> getInstance(Element[] leftElems,
            Element[] rightElems) {
     ...
     */
    public boolean hasOnlyStandardUnit() {
        for (int i = 0; i < _elements.length; i++) {
    ...
    private final static class Element implements Serializable {
         ...
         */
        public Element(Unit<?> unit, int pow, int root) {
            _unit = unit;
            _pow = pow;
            _root = root;
        }
	...
	public void set_hashCode(int _hashCode) {
