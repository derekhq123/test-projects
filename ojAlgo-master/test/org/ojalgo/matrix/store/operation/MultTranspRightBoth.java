...
public class MultTranspRightBoth {
    ...
    public static void main(final String[] args) throws RunnerException {
        LinearAlgebraBenchmark.run(MultTranspRightBoth.class);
    }
    ...
    @Benchmark
    public PrimitiveDenseStore multiplyBoth() {
        MB.invoke(product, left, complexity, right.transpose());
        return product;
    };

    @Benchmark
    public PrimitiveDenseStore multiplyDynamic() {
        product.fillByMultiplying(left, right.transpose());
        return product;
    }

    @Benchmark
    public PrimitiveDenseStore multiplyRight() {
        MR.invoke(product.data, left.data, complexity, right.transpose());
        return product;
    }

    @Benchmark
    public PrimitiveDenseStore multiplyStatic() {
        MultiplyRight.invoke(product.data, 0, complexity, left.data, complexity, right.transpose());
        return product;
    }
	...
	public void setMB(PrimitiveDenseStore.PrimitiveMultiplyBoth MB) {
	    this.MB = MB;
	}

	public void setRight(PrimitiveDenseStore right) {
		this.right = right;
	}
