...
public class MultTranspRight {
    ...
    public static void main(final String[] args) throws RunnerException {
        LinearAlgebraBenchmark.run(MultTranspRight.class);
    }
    ...
    @Benchmark
    public PrimitiveDenseStore multiplyRightDynamicInt() {
        product.fillByMultiplying(left, rightD.transpose());
        return product;
    };

    @Benchmark
    public PrimitiveDenseStore multiplyRightDynamicPre() {
        product.fillByMultiplying(left, rightT);
        return product;
    }

    @Benchmark
    public PrimitiveDenseStore multiplyRightFixedInt() {
        MR.invoke(product.data, left.data, complexity, rightD.transpose());
        return product;
    }

    @Benchmark
    public PrimitiveDenseStore multiplyRightFixedPre() {
        MR.invoke(product.data, left.data, complexity, rightT);
        return product;
    }

    @Benchmark
    public PrimitiveDenseStore multiplyRightStandardInt() {
        MultiplyRight.invoke(product.data, 0, complexity, left.data, complexity, rightD.transpose());
        return product;
    }

    @Benchmark
    public PrimitiveDenseStore multiplyRightStandardPre() {
    ...
    public void setup() {
        ...
        rightD = PrimitiveDenseStore.FACTORY.makeFilled(complexity, complexity, new Normal());
	...
	public void setMR(PrimitiveDenseStore.PrimitiveMultiplyRight MR) {
	    this.MR = MR;
	}

	public void setProduct(PrimitiveDenseStore product) {
		this.product = product;
	}

	public void setLeft(PrimitiveDenseStore left) {
		this.left = left;
	}
