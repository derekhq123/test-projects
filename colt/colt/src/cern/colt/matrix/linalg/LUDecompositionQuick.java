...
*/
public boolean isNonsingular(DoubleMatrix2D matrix) {
	int m = matrix.rows();
...
@exception  IllegalArgumentException  if <tt>A.rows() < A.columns()</tt>.
*/
public void solveOld(DoubleMatrix2D B) {
	algebra.property().checkRectangular(LU);
	int m = m();
	int n = n();
	if (B.rows() != m) throw new IllegalArgumentException("Matrix row dimensions must agree.");
...
public void setWorkDouble(double[] workDouble) {
	this.workDouble = workDouble;
}
public void setAlgebra(Algebra algebra) {
	this.algebra = algebra;
}
public void setIsNonSingular(boolean isNonSingular) {
