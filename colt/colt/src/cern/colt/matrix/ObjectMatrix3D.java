 ...
 */
public boolean haveSharedCells(ObjectMatrix3D other) {
 ...
 */
public abstract ObjectMatrix2D like2D(int rows, int columns, int rowZero, int columnZero, int rowStride, int columnStride);
/**
 ...
 */
public boolean xforEach(final cern.colt.function.ObjectProcedure procedure) {
	for (int slice=slices; --slice >= 0;) {
		for (int row=rows; --row >= 0;) {
			for (int column=columns; --column >= 0;) {
 ...
 * @return <tt>false</tt> if the procedure stopped before all elements where iterated over, <tt>true</tt> otherwise. 
 */
public boolean xforEachCoordinate(final cern.colt.function.IntIntIntProcedure procedure) {
	for (int column=columns; --column >= 0;) {
		for (int slice=slices; --slice >=0;) {
			for (int row=rows; --row >= 0;) {
				if (!procedure.apply(slice,row,column)) return false;
