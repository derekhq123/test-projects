...
public abstract class AbstractTestCase extends TestCase
{
     ...
     */
    public File createTestdataFile(final String name)
    {
        return new File(TESTDATA, name);
    }

    public JavaClass getTestClass(final String name) throws ClassNotFoundException
    {
        return SyntheticRepository.getInstance().loadClass(name);
    }

    public Method getMethod(final JavaClass cl, final String methodname)
    {
     ...
     * @return
     */
    public boolean wipe(final String name)
    {
     ...
     */
    public boolean wipe(final String dir, final String name)
    {
    ...
    public Attribute[] findAttribute(final String name, final JavaClass clazz)
    {
    ...
    public Attribute findAttribute(final String name, final Attribute[] all)
    {
        final List<Attribute> chosenAttrsList = new ArrayList<>();
        for (final Attribute element : all) {
            if (verbose) {
    ...
    public String dumpAttributes(final Attribute[] as)
    {
    ...
    public String dumpAnnotationEntries(final AnnotationEntry[] as)
    {
        final StringBuilder result = new StringBuilder();
        result.append("[");
        for (int i = 0; i < as.length; i++)
    ...
    public String dumpAnnotationEntries(final AnnotationEntryGen[] as)
    {
        final StringBuilder result = new StringBuilder();
        result.append("[");
        for (int i = 0; i < as.length; i++)
